...
static int write_csv_header(FILE *fout, const dbf_field_t *fields, int nfields, encoding_mode_t enc_mode)
{
...
static int write_csv_row(FILE *fout, const unsigned char *record,
                         const dbf_field_t *fields, int nfields, encoding_mode_t enc_mode)
{
                              ...
                              char *err_buf, size_t err_sz)
{
    ...
    if (write_csv_header(fout, fields, nfields, enc_mode) != 0) {
    ...
    while (records_written < records_total) {
        ...
        for (size_t r = 0; r < got; r++) {
            ...
            if (write_csv_row(fout, rec, fields, nfields, enc_mode) != 0) {
